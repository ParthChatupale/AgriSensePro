...
OUTPUT_BASE = os.path.join(BASE_DIR, "ndvi", "data", "ndvi_jobs")
...
def write_stats(ndvi, out_path):
    ...
    if valid_pixels == 0:
        ...
        stats = {
            "min": safe_num(np.nanmin(ndvi)),
            "max": safe_num(np.nanmax(ndvi)),
            "mean": safe_num(np.nanmean(ndvi)),
...
def run_ndvi(lat, lon, radius_m=DEFAULT_RADIUS_M, date=None, out_prefix="ndvi"):
    ...
    # Prepare output directory per prefix
    out_dir = os.path.join(OUTPUT_BASE, out_prefix)
    ...
    # Save raw arrays
    np.save(os.path.join(out_dir, f"{out_prefix}_raw.npy"), ndvi_native)
    np.save(os.path.join(out_dir, f"{out_prefix}_masked_raw.npy"), ndvi_masked)
    np.save(os.path.join(out_dir, f"{out_prefix}_visual.npy"), ndvi_visual)

    # Save images
    save_png(ndvi_native, os.path.join(out_dir, f"{out_prefix}_native.png"))
    save_png(ndvi_masked, os.path.join(out_dir, f"{out_prefix}_masked.png"), alpha_mask=mask)
    save_png(ndvi_visual, os.path.join(out_dir, f"{out_prefix}_visual.png"))

    # Write stats for masked NDVI
    stats = write_stats(ndvi_masked, os.path.join(out_dir, f"{out_prefix}_stats.json"))
